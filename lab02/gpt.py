...
from typing import Optional, Generator, Any, Protocol
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.flash:
            ...
            scores = scores.masked_fill(self.tril[:, :, :T, :T] == 0, -torch.inf)  # type:ignore
...
def train(
    ...
    data_train = torch.tensor(tokenizer.encode(text_train), dtype=torch.long)
    data_valid = torch.tensor(tokenizer.encode(text_valid), dtype=torch.long)

    print("Dataset sizes (in tokens):", data_train.size(), data_valid.size())
    ...
    for epoch in (pbar := trange(num_epochs)):
        ...
        if epoch % log_period == 0 or epoch == num_epochs - 1:
            loss = evaluate(model, data_valid, num_evals, batch_size, device)
            ...
            torch.save(
                ...
                f"checkpoints/{name}/chk_{epoch}.pt",
            )

            with open(f"checkpoints/{name}/out_{epoch}.txt", "w", encoding="utf-8") as f:
